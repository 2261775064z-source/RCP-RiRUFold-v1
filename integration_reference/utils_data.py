...
def _find_split_layout(base_dir, mode):
    """Resolve common ISTD layouts without silently inventing a split."""
    base_dir = osp.abspath(base_dir)
    split_names = ['trainval', 'train'] if mode == 'train' else ['test', 'val']

    # Standard layout: dataset/{trainval,test}/{images,masks}.
    for split_name in split_names:
        split_dir = osp.join(base_dir, split_name)
        if osp.isdir(osp.join(split_dir, 'images')) and osp.isdir(osp.join(split_dir, 'masks')):
            names = sorted(
                filename for filename in os.listdir(osp.join(split_dir, 'images'))
                if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp'))
            )
            return osp.join(split_dir, 'images'), osp.join(split_dir, 'masks'), names

    # Flat layout with an explicit split file, used by IRSTD-1K.
    image_dir = osp.join(base_dir, 'images')
    mask_dir = osp.join(base_dir, 'masks')
    # Official IRSTD-1K releases use these directory names together with
    # trainval.txt and test.txt instead of images/masks.
    if not (osp.isdir(image_dir) and osp.isdir(mask_dir)):
        image_dir = osp.join(base_dir, 'IRSTD1k_Img')
        mask_dir = osp.join(base_dir, 'IRSTD1k_Label')
    if not (osp.isdir(image_dir) and osp.isdir(mask_dir)):
        raise FileNotFoundError(
            'Unsupported dataset layout. Expected dataset/{trainval,test}/{images,masks} '
            'or flat images/masks with trainval.txt and test.txt.'
        )
    list_candidates = ['trainval.txt', 'train.txt'] if mode == 'train' else ['test.txt', 'val.txt', 'test1.txt']
    list_path = next((osp.join(base_dir, name) for name in list_candidates if osp.isfile(osp.join(base_dir, name))), None)
    if list_path is None:
        raise FileNotFoundError(
            f'No split file found for mode={mode} in {base_dir}. '
            'Do not evaluate a flat dataset without an explicit train/test split.'
        )
    names = [line.strip() for line in open(list_path, 'r', encoding='utf-8') if line.strip()]
    return image_dir, mask_dir, names


def _resolve_image_path(directory, name):
    """Resolve a split-list stem or filename against a directory."""
    name = name.strip()
    candidates = [osp.join(directory, name)]
    if not osp.splitext(name)[1]:
        candidates.extend(osp.join(directory, name + ext) for ext in ('.png', '.jpg', '.jpeg', '.bmp'))
    for candidate in candidates:
        if osp.isfile(candidate):
            return candidate
    raise FileNotFoundError(f'Cannot find image/mask file for split entry: {name} in {directory}')


class _FlexibleISTDDataset(Data.Dataset):
    """Shared loader for directory-split and list-file-split ISTD datasets."""

    def __init__(self, base_dir, mode='train', base_size=256, augment=False):
        assert mode in ['train', 'test']
        self.image_dir, self.mask_dir, self.names = _find_split_layout(base_dir, mode)
    ...
    def __getitem__(self, i):
        name = self.names[i]
        img = cv2.imread(_resolve_image_path(self.image_dir, name), 0)
        mask = cv2.imread(_resolve_image_path(self.mask_dir, name), 0)
        ...
        img = img.reshape(1, self.base_size, self.base_size).astype(np.float32) / 255.0
        mask_max = np.max(mask)
        mask = mask.reshape(1, self.base_size, self.base_size).astype(np.float32)
        if mask_max > 0:
            mask = mask / mask_max
...
class NUDTDataset(Data.Dataset):
    ...
    def __init__(self, base_dir=r'D:/WFY/datasets/NUDT',
        ...
        self._flexible = _FlexibleISTDDataset(base_dir, mode, base_size, augment=False)
        self.base_size = base_size
        self.names = self._flexible.names
        self.image_dir = self._flexible.image_dir
        self.mask_dir = self._flexible.mask_dir
        return
...
class IRSTD1kDataset(Data.Dataset):
    ...
    def __init__(self, base_dir=r'D:/WFY/datasets/IRSTD-1k',
        ...
        # Supports both the repository's flat list-file layout and the
        # trainval/test directory layout used by some prepared copies.
        nested = osp.join(base_dir, 'IRSTD-1k')
        if osp.isdir(nested):
            base_dir = nested
        self._flexible = _FlexibleISTDDataset(base_dir, mode, base_size, augment=False)
        self.base_size = base_size
...
class SirstAugDataset(Data.Dataset):
    ...
    def __init__(self, base_dir=r'/Users/tianfangzhang/Program/DATASETS/sirst_aug',
        ...
        self._flexible = _FlexibleISTDDataset(base_dir, mode, base_size, augment=(mode == 'train'))
        self.base_size = base_size
        self.names = self._flexible.names
        self.image_dir = self._flexible.image_dir
        self.mask_dir = self._flexible.mask_dir
